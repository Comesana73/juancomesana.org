...
- `assets/documents/` contains CV PDFs and similar documents. `cv.pdf` is built from `cv.yaml` in the iCloud `inprogress/cv/` folder; `cv-latex.pdf` and `s/cv*.pdf` are copies of the full CV kept so old links still work.
